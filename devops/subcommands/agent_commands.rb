...
class AgentCommands < Thor
  ...
  def create(topic)
    ...
    execute_command("git fetch origin #{from_branch}", quiet: !options[:verbose])
  ...
  def destroy(topic)
    ...
    unless Dir.exist?(worktree_path)
      ...
      # Check if it's registered but missing
    ...
    success = execute_command("git worktree remove #{force_flag} #{worktree_path}")
    ...
    if success
      ...
      if options[:delete_branch] && branch_name
        ...
        if execute_command("git branch -D #{branch_name}")
  ...
  def get_worktree_dir
    ...
    if configured_dir
      ...
      # Default to ./worktrees in the repo root
  ...
  def get_default_branch
    ...
    if branches.any? { |b| b.include?("origin/#{default}") }
      ...
      # Just return what's configured and let git fail if it doesn't exist
      default
    end
  end
  ...
  def copy_env_file(worktree_path)
    ...
    # Get the repo root to find the source env file
    repo_root = `git rev-parse --show-toplevel 2>/dev/null`.strip
  ...
  def get_worktree_list
    ...
    output.split("\n\n").each do |wt_block|
      ...
      # Skip the main worktree (usually at the root)
      next if wt_path == `git rev-parse --show-toplevel`.strip
      
      # Only include worktrees in our configured directory
